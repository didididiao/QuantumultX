...
let shopName=''
const token=[
'7D7E23BB728C9D7425C776BDC8943D10',//6.20.5R
'FC18859B46B04ED989F3D7C3C4E744A2',//6.20,30
'A9243E5620D880D32D1757B2089C04DB',//6.22,100
'E513A50A3F12093062F423A08438C4B2',//6.22,30
]
...
if ($.isNode()) {
  ...
  if (process.env.JD_DEBUG && process.env.JD_DEBUG === 'false') console.log = () => {};
      ...
      cookiesArr = [...jsonParse($.getdata('CookiesJD') || "[]").map(item => item.cookie)].filter(item => !!item);
}
  ...
  return new Promise(resolve => {
    const options = {
      ...
      headers: {
        ...
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 14_7_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.11(0x18000b23) NetType/WIFI Language/zh_CN"
      }
    }
...
async function showMsg() {
  if ($.isNode()) {
    $.msg($.name, '', `【京东账号${$.index}】\n${message}`);
    await notify.sendNotify(`${$.name} - 账号${$.index} `, `【京东账号${$.index}】\n${message}`);
  }
}
